define(['jquery'], function($){
  ...
  var defaultDiacriticsRemovalMap = [
    ...
    {'base':'D', 'letters':/[\u0044\u24B9\uFF24\u1E0A\u010E\u1E0C\u1E10\u1E12\u1E0E\u0110\u018B\u018A\u0189\uA779\u00F0]/g},
  ...
  function process(term, data, ops){
    ...
    for(var i=0; i<res.length; i++){
      ...
      for(var locale in res[i]['prefLabel']){
        ...
        var valX       = removeDiacritics(val);
        var match      = valX.match(re);
        var matchIndex = valX.indexOf(match);
        var inLangs    = ops.languages.indexOf(locale) > -1;

        if(inLangs){
          res[i].directHit   = res[i].directHit ? res[i].directHit : val;
          if(match){
            res[i].directHitPreMatch  = val.substr(0, valX.indexOf(match));
            res[i].directHitPostMatch = val.substr(matchIndex + term.length);
            res[i].directMatch        = val.substr(matchIndex, term.length);
          }
          else{
            res[i].unmatched = val;
          }
          ...
          res[i].indirectHitPreMatch  = val.substr(0, matchIndex);
          res[i].indirectHitPostMatch = val.substr(matchIndex + term.length);
          res[i].indirectMatch        = val.substr(matchIndex, term.length);
        }
      }
